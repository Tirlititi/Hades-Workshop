...
vector<SortedChoiceItemModel> HADES_STRING_SPELL_MODEL = {
	...
	{ 90,  L"Might", SPELL_TARGET_TYPE_ANY, SPELL_TARGET_AMOUNT_ONE, 0 },
	...
	{ 414, L"Stock Break-2", SPELL_TARGET_TYPE_ANY, SPELL_TARGET_AMOUNT_GROUP, 0 },
	{ 459, L"Stock Break-3", SPELL_TARGET_TYPE_ANY, SPELL_TARGET_AMOUNT_GROUP, 0 },
	...
	{ 100, L"[Attack-1...]", SPELL_TARGET_TYPE_ANY, SPELL_TARGET_AMOUNT_ONE, 0 },
...
SortedChoiceItemModel& GetSpellModelInfo(uint16_t modelid) {
	for (unsigned int i = 0; i < HADES_STRING_SPELL_MODEL.size(); i++)
		if (HADES_STRING_SPELL_MODEL[i].id == modelid)
			return HADES_STRING_SPELL_MODEL[i];
	if (modelid > SPELL_MODEL_ATTACK_ID && modelid < SPELL_MODEL_ATTACK_ID + SPELL_MODEL_ATTACK_AMOUNT)
		return GetSpellModelInfo(SPELL_MODEL_ATTACK_ID);
	if (modelid > SPELL_MODEL_WEAPON_ID && modelid < SPELL_MODEL_WEAPON_ID + SPELL_MODEL_WEAPON_AMOUNT)
		return GetSpellModelInfo(SPELL_MODEL_WEAPON_ID);
	return HADES_STRING_SPELL_MODEL[0];
}

vector<SpellAnimSequenceCode> SPELLANIM_OPCODE = {
	...
	{ 0x05, L"Load chunk", L"Load a chunk of SFX datas.\n - 1st arg: chunk index\n - 2nd arg: unknown", { SAAT_INTEGER, SAAT_INTEGER } },
	...
	{ 0x23, L"Setup camera", L"Add a camera to the list of variable cameras or clear that list.\n - 1st arg: camera index or 255 (clear)\n - 2nd arg: unknown", { SAAT_INTEGER, SAAT_INTEGER } },
	...
	{ 0x29, L"Play camera", L"Play a camera movement.\n - 1st arg: camera index\n - 2nd arg: type (0: by index, 3: variable camera...)", { SAAT_INTEGER, SAAT_INTEGER } },
	...
	{ 0x2C, L"Change FPS", L"Change the framerate (only works with divisors of 60).\n - 1st arg: target FPS\n - 2nd arg: unknown", { SAAT_INTEGER, SAAT_INTEGER } },
	...
	{ 0x64, L"Play Freya's casting anim", L"Play the (special) casting animation of Freya instead of her regular (jumping) casting animation.\n - 1st arg: unknown (maybe speed?)\n - 2nd arg: animation 0 (ANH_MAIN_B0_011_201) or 1 (ANH_MAIN_B0_011_202). For Freya's natural spell animations, the switch is not with 0/1 but with other values.", { SAAT_INTEGER, SAAT_INTEGER } },
	{ 0x65, L"0x65", L"Unknown effect.\n - 1st arg: unknown\n - 2nd arg: unknown", { SAAT_INTEGER, SAAT_INTEGER } },
	{ 0x69, L"Move before", L"Instantly move a character in front of another character.\n - 1st arg: character to move\n - 2nd arg: other character", { SAAT_CHARACTER, SAAT_CHARACTER } },
...
SpellAnimSequenceCode& GetSpellAnimSequenceCode(uint8_t code) {
	for (unsigned int i = 0; i < SPELLANIM_OPCODE.size(); i++)
		if (SPELLANIM_OPCODE[i].id == code)
			return SPELLANIM_OPCODE[i];
	return SPELLANIM_OPCODE[SPELLANIM_OPCODE.size() - 1];
}

vector<SpellAnimSequenceArgumentString> SPELLANIM_ARGUMENT_STRING = {
	...
	{ SAAT_ANIMATION, 2,	L"hitted" },
	{ SAAT_ANIMATION, 3,	L"hitted strong" },
	{ SAAT_ANIMATION, 4,	L"stand KO" },
	{ SAAT_ANIMATION, 5,	L"weak -> stand" },
	{ SAAT_ANIMATION, 6,	L"KO -> weak" },
	{ SAAT_ANIMATION, 7,	L"stand -> weak" },
	{ SAAT_ANIMATION, 8,	L"weak -> KO" },
	{ SAAT_ANIMATION, 9,	L"stand ready" },
	{ SAAT_ANIMATION, 10,	L"stand -> ready" },
	{ SAAT_ANIMATION, 11,	L"weak -> ready" },
	{ SAAT_ANIMATION, 12,	L"ready -> defend" },
	{ SAAT_ANIMATION, 13,	L"stand defend" },
	{ SAAT_ANIMATION, 14,	L"defend -> stand" },
	{ SAAT_ANIMATION, 15,	L"cover" },
	{ SAAT_ANIMATION, 16,	L"dodge" },
	{ SAAT_ANIMATION, 17,	L"flee" },
	{ SAAT_ANIMATION, 18,	L"victory" },
	{ SAAT_ANIMATION, 19,	L"victory loop" },
	{ SAAT_ANIMATION, 20,	L"ready -> attack" },
	{ SAAT_ANIMATION, 21,	L"attack run" },
	{ SAAT_ANIMATION, 22,	L"attack 1" },
	{ SAAT_ANIMATION, 23,	L"attack 2" },
	{ SAAT_ANIMATION, 24,	L"jump back 1" },
	{ SAAT_ANIMATION, 25,	L"jump back 2" },
	{ SAAT_ANIMATION, 26,	L"ready -> cast" },
	...
	{ SAAT_ANIMATION, 33,	L"special cast" },
...
vector<EnemySequenceCode> ENEMYANIM_OPCODE = {
	...
	{ 0x0C, L"Resize", L"Resize the caster.\n - 1st arg: Size factor (4096 and 65535 keep the size)\n - 2nd arg: Time", 2, new int[2]{ 2, 1 }, new Enemy_Animation_Argument_Type[2]{ EAAT_INTEGER, EAAT_TIME } },
...
EnemySequenceCode& GetEnemySequenceCode(uint8_t code) {
	for (unsigned int i = 0; i < ENEMYANIM_OPCODE.size(); i++)
