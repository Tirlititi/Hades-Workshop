...
bool ConvertModelToFBX(ModelDataStruct& model, FbxManager*& sdkmanager, FbxScene*& sdkscene, string texturefolder, bool includeanims = true);
...
//fout.open("aaaa.txt",ios::out|ios::app); fout << "bone_amount: " << (unsigned int)bone_amount << endl; fout.close();
	bone.resize(bone_amount);
	for (i = 0; i < bone_amount; i++)
		for (j = 0; j < 4; j++)
			for (k = 0; k < 4; k++)
				bone[i].transform_matrix.value[j][k] = (k == 3 ? -ReadFloat(f) : ReadFloat(f));
...
void ModelMaterialData::Read(fstream& f, UnityArchiveMetaData& metadata) {
	const int FIELD_NAME_MAX_LEN = 0x40;
	uint32_t fieldlen;
	char fieldbuffer[FIELD_NAME_MAX_LEN];
	...
	while (fieldlen < FIELD_NAME_MAX_LEN && !f.eof()) {
...
void ModelAnimationData::Write(fstream& f) {
	...
	#define MACRO_ANIM_END_WRITE(TYPE) \
		...
		else if (TYPE == 4) { for (i = 0; i < 5; i++) WriteFloat(f, 1.0f); for (i = 0; i < 5; i++) WriteLong(f, 0); } \
		j = TYPE == 3 ? 0x34 : 0; \
		WriteLong(f, j); for (i = 0; i < j; i++) WriteLong(f, 0); \
		j = TYPE == 3 ? 0x07 : 0; \
		WriteLong(f, j); for (i = 0; i < 5 * j; i++) WriteLong(f, 0); \
		WriteFloat(f, 1.0f); \
		for (i = 0; i < 4; i++) WriteFloat(f, TYPE == 5 ? 0.0f : 1.0f);
	...
	for (i = 0; i < hierarchy->node_list.size(); i++) {
		if (hierarchy->node_list[i]->node_type == 23) {
			...
			for (j = 0; j < node->child_material_amount; j++) {
				...
				if (matdata.maintex.file_name.empty())
					matdata.maintex.file_name = matdata.name + ".png";
...
int ModelDataStruct::Export(const char* outputname, int format, bool exportanims) {
    FbxManager* sdkmanager = NULL;
    FbxScene* sdkscene = NULL;
	string foldername = outputname;
	size_t seppos = foldername.find_last_of("/\\");
	if (seppos != string::npos)
		foldername = foldername.substr(0, seppos + 1);
	filesystem::path workingdir = filesystem::current_path();
	filesystem::current_path(foldername);
    InitializeSdkObjects(sdkmanager, sdkscene);
    ConvertModelToFBX(*this, sdkmanager, sdkscene, foldername, exportanims);
...
bool ConvertModelToFBX(ModelDataStruct& model, FbxManager*& sdkmanager, FbxScene*& sdkscene, string texturefolder, bool includeanims) {
	...
	FbxNode* lSceneRootNode = sdkscene->GetRootNode();
	FbxNode* lRootNode = NULL;
	...
	if (includeanims && model.animation.size() > 0 && !hasbones) {
	...
	for (i = 0; i < hierarchynode.size(); i++) {
		if (hierarchynode[i]->node_type == 4) {
			...
			if (includeanims || nodename.substr(0, 4).compare("bone") != 0) {
	...
	const FbxSkeleton::EType ROOT_BONE_TYPE = FbxSkeleton::eLimbNode;
	for (i = 0; i < hierarchynode.size(); i++) {
		...
		} else if (includeanims && hierarchynode[i]->node_type == 137 && hasbones) {
					...
					if (newbone) { \
						...
						if (boneobj) { \
							...
							if (parenttransf->parent_transform == NULL || parenttransf->parent_transform == model.hierarchy->root_node) \
								lSkeleton->SetSkeletonType(ROOT_BONE_TYPE); \
							else \
		...
		if (hierarchynode[i]->node_type == 1) {
			...
			if (meshindex >= 0) {
				...
				for (j = 0; j < material.size(); j++) {
					if (material[j].maintex.file_name.empty())
						material[j].maintex.file_name = material[j].name + ".png";
				...
				if (includeanims && (model.animation.size() > 0 || hasbones)) {
					...
					if (model.animation.size() > 0 && !hasbones) {
						...
						lTransformMatrix = lMeshNode->EvaluateGlobalTransform();
						...
						for (j = 0; j < mesh.bone_amount; j++) {
							...
							for (k = 0; k < lMultiSkeletonName.size(); k++)
								if (lMultiSkeletonName[k] == mesh.bone[j].scoped_name) {
									...
									lTransformLinkMatrix = lMultiSkeleton[k]->GetNode()->EvaluateGlobalTransform();
									break;
								}
...
bool ConvertFBXToModel(ModelDataStruct& model, FbxManager*& sdkmanager, FbxScene*& sdkscene, bool retrieveanims) {
	...
	for (i = 0; i < lNodeList.size(); i++) {
		...
		if (lCurrentNode->GetNodeAttribute() != NULL && lCurrentNode->GetNodeAttribute()->GetAttributeType() == FbxNodeAttribute::EType::eMesh) {
			...
			for (j = 0; (int)j < lMesh->GetPolygonCount(); j++) {
			...
			if (lSkin != NULL) {
				...
				for (j = 0; j < newmesh.vertice_attachment_amount; j++) {
					...
					for (k = 0; (int)k < lSkin->GetClusterCount(); k++) {
						...
						for (l = 0; (int)l < lSkeletonCluster->GetControlPointIndicesCount(); l++) {
							if (lPtsIndex[l] == j) {
								for (m = 0; m < 4; m++)
