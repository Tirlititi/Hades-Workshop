...
int WorldMapDataStruct::SetName(unsigned int placeid, wstring newvalue, SteamLanguage lang) {
	...
	place_name[placeid].SetValue(newvalue, lang);
	return 0;
}
...
void WorldMapDataSet::Load(fstream& ffbin, ClusterSet& clusset) {
	...
	if (GetGameType() == GAME_TYPE_PSX) {
		...
		text_data[0]->block_id = STEAM_WORLD_MAP_TEXT_ID;
		text_data[0]->base_amount = 0;
		if (auto search = HADES_STRING_TEXT_BLOCK_NAME.find(STEAM_WORLD_MAP_TEXT_ID); search != HADES_STRING_TEXT_BLOCK_NAME.end())
			text_data[0]->base_amount = search->second.base_text_count;
		textamount = max<uint16_t>(textamount, text_data[0]->base_amount);
		text_data[0]->text.resize(textamount);
		for (i = 0; i < textamount; i++)
			text_data[0]->text[i].id = i;
//		text_data[0]->size = fsize;
		...
		for (lang = 0; lang < STEAM_LANGUAGE_AMOUNT; lang++) {
			...
			for (i = 0; i < text_lang_amount[lang]; i++)
				SteamReadFF9String(ffbin, text_data[0]->text[i].txt, lang);
			while (i < textamount) // Now, all the texts are initialised in all languages (except in STEAM_SINGLE_LANGUAGE_MODE)
				text_data[0]->text[i++].txt.SetValue(L"[STRT=0,1]", lang);
		}
		for (i = 1; i < amount; i++)
...
int WorldMapDataSet::GetSteamTextSize(SteamLanguage lang) {
	int res = 0;
...
void WorldMapDataSet::WriteSteamText(fstream& ffbin, SteamLanguage lang) {
	for (unsigned int i = 0; i < WORLD_MAP_PLACE_AMOUNT; i++)
		SteamWriteFF9String(ffbin, world_data->place_name[i], lang);
}

void WorldMapDataSet::WriteSteamTextPatch(fstream& fileout, wxArrayString basefile, SteamLanguage lang) {
	int lastid = -1;
	for (unsigned int i = 0; i < WORLD_MAP_PLACE_AMOUNT; i++) {
		if (!world_data->place_name[i].multi_lang_init[lang])
			continue;
		wxString str = lang == GetSteamLanguage() ? _(world_data->place_name[i].str) : _(world_data->place_name[i].multi_lang_str[lang]);
		if (i < basefile.GetCount() && basefile[i].IsSameAs(str))
			continue;
		if (i != lastid + 1)
			SteamWriteFF9StringLight(fileout, L"[TXID=" + to_wstring(i) + L"]");
		SteamWriteFF9String(fileout, world_data->place_name[i], lang);
		lastid = i;
	}
}
					...
					if (clustersize <= clus->size + clus->extra_size) {
						...
						while (chunktype != CHUNK_SPECIAL_END) {
							...
							if (chunktype == CHUNK_TYPE_SCRIPT) {
								...
								if (loadmain) {
									script[j]->ReadHWS(ffhws, true, STEAM_LANGUAGE_AMOUNT);
									script[j]->SetSize(chunksize);
								}
			...
			if (GetGameType() != GAME_TYPE_PSX) {
				...
				for (lang = 0; lang < STEAM_LANGUAGE_AMOUNT; lang++)
					if (hades::STEAM_LANGUAGE_SAVE_LIST[lang]) {
						...
						HWSWriteLong(ffhws, GetSteamTextSize(lang));
						for (j = 0; j < WORLD_MAP_PLACE_AMOUNT; j++)
