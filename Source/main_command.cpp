...
void InitConstEditableField() {
	...
	const_editable_field = new editable_field*[EDITABLE_FIELD_COUNT] {
		...
		new editable_field_numerical(L"status_set.status", [](SaveSet& sv, FieldSelector& sel) { return sel.status_set_index < STATUS_SET_AMOUNT; }, [](SaveSet& sv, FieldSelector& sel, long long val) { SetupStatusList(sv.spellset->status_set[sel.status_set_index].status_list, val); sv.sectionmodified[DATA_SECTION_SPELL] = true; }, [](SaveSet& sv, FieldSelector& sel) { return GetStatusBitList(sv.spellset->status_set[sel.status_set_index].status_list); }),
		...
		new editable_field_numerical(L"ability_list.slot.ability", [](SaveSet& sv, FieldSelector& sel) { return sel.ability_list_index < ABILITY_SET_AMOUNT && sel.ability_list__slot_index < ABILITY_SET_CAPACITY; }, [](SaveSet& sv, FieldSelector& sel, long long val) { sv.statset->ability_list[sel.ability_list_index].entry[sel.ability_list__slot_index].ability.id = val; sv.sectionmodified[DATA_SECTION_STAT] = true; }, [](SaveSet& sv, FieldSelector& sel) { return sv.statset->ability_list[sel.ability_list_index].entry[sel.ability_list__slot_index].ability.id; }),
		new editable_field_numerical(L"ability_list.slot.ap_cost", [](SaveSet& sv, FieldSelector& sel) { return sel.ability_list_index < ABILITY_SET_AMOUNT && sel.ability_list__slot_index < ABILITY_SET_CAPACITY; }, [](SaveSet& sv, FieldSelector& sel, long long val) { sv.statset->ability_list[sel.ability_list_index].entry[sel.ability_list__slot_index].ap_cost = val; sv.sectionmodified[DATA_SECTION_STAT] = true; }, [](SaveSet& sv, FieldSelector& sel) { return sv.statset->ability_list[sel.ability_list_index].entry[sel.ability_list__slot_index].ap_cost; }),
		...
		new editable_field_numerical(L"item.status", [](SaveSet& sv, FieldSelector& sel) { return sel.item_index < ITEM_AMOUNT&& sv.itemset->item[sel.item_index].usable_id >= 0; }, [](SaveSet& sv, FieldSelector& sel, long long val) { SetupStatusList(sv.itemset->usable[sv.itemset->item[sel.item_index].usable_id].status, val); sv.sectionmodified[DATA_SECTION_ITEM] = true; }, [](SaveSet& sv, FieldSelector& sel) { return GetStatusBitList(sv.itemset->usable[sv.itemset->item[sel.item_index].usable_id].status); }),
		...
		new editable_field_numerical(L"battle.enemy.status_immune", [](SaveSet& sv, FieldSelector& sel) { int index = sv.enemyset->GetIndexById(sel.battle_id); if (index < 0) return false; return sel.battle__enemy_index < sv.enemyset->battle[index]->stat_amount; }, [](SaveSet& sv, FieldSelector& sel, long long val) { int index = sv.enemyset->GetIndexById(sel.battle_id); SetupStatusList(sv.enemyset->battle[index]->stat[sel.battle__enemy_index].status_immune, val); sv.enemyset->battle[index]->MarkDataModified(); sv.sectionmodified[DATA_SECTION_ENMY] = true; }, [](SaveSet& sv, FieldSelector& sel) { return GetStatusBitList(sv.enemyset->battle[sv.enemyset->GetIndexById(sel.battle_id)]->stat[sel.battle__enemy_index].status_immune); }),
		new editable_field_numerical(L"battle.enemy.status_auto", [](SaveSet& sv, FieldSelector& sel) { int index = sv.enemyset->GetIndexById(sel.battle_id); if (index < 0) return false; return sel.battle__enemy_index < sv.enemyset->battle[index]->stat_amount; }, [](SaveSet& sv, FieldSelector& sel, long long val) { int index = sv.enemyset->GetIndexById(sel.battle_id); SetupStatusList(sv.enemyset->battle[index]->stat[sel.battle__enemy_index].status_auto, val); sv.enemyset->battle[index]->MarkDataModified(); sv.sectionmodified[DATA_SECTION_ENMY] = true; }, [](SaveSet& sv, FieldSelector& sel) { return GetStatusBitList(sv.enemyset->battle[sv.enemyset->GetIndexById(sel.battle_id)]->stat[sel.battle__enemy_index].status_auto); }),
		new editable_field_numerical(L"battle.enemy.status_initial", [](SaveSet& sv, FieldSelector& sel) { int index = sv.enemyset->GetIndexById(sel.battle_id); if (index < 0) return false; return sel.battle__enemy_index < sv.enemyset->battle[index]->stat_amount; }, [](SaveSet& sv, FieldSelector& sel, long long val) { int index = sv.enemyset->GetIndexById(sel.battle_id); SetupStatusList(sv.enemyset->battle[index]->stat[sel.battle__enemy_index].status_initial, val); sv.enemyset->battle[index]->MarkDataModified(); sv.sectionmodified[DATA_SECTION_ENMY] = true; }, [](SaveSet& sv, FieldSelector& sel) { return GetStatusBitList(sv.enemyset->battle[sv.enemyset->GetIndexById(sel.battle_id)]->stat[sel.battle__enemy_index].status_initial); }),
...
bool CommandFrame::ProcessCommandConfig(wxArrayString args) {
	...
	if (args.GetCount() < 2) {
		...
		} else if (args[0].IsSameAs("prefer_export_as_patches")) {
			hades::PREFER_EXPORT_AS_PATCHES = args[1].IsSameAs("true", false) || args[1].IsSameAs("1");
			WriteToOutput(_("Configure prefer_export_as_patches to ") + (hades::PREFER_EXPORT_AS_PATCHES ? _("true\n") : _("false\n")));
