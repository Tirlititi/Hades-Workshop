	...
	do{
		...
		if(read!=0){
			if(read==eightmb){
				...
				changes=PPFWriteChanges(x, y, eightmb, chunk);
			} else {
				fread(y, 1, read, mod);
				changes=PPFWriteChanges(x, y, read, chunk);
			}
		}
		chunk+=eightmb;
		found+=changes;
...
// Compatibility note : blocks are not 8 Mbit long for my use.
...
int PPFCreateHeader(unsigned char binblock[1024]){
	unsigned char method=0x02, description[128], dummy=0, i=0;
...
*/
	Arg.ppfname=ppfpath;
	Arg.undo=undo;
	Arg.blockcheck=blockcheck;
	Arg.imagetype=0;
	Arg.fileid=0;
	Arg.desc=strlen(description)>0;
